...
resource "null_resource" "s3_sync" {
    ...
    provisioner "local-exec" {
        command = "aws s3 sync ../public/ s3://${var.lambda_function_name}-${local.account_id}-${local.region}-static/public/ && aws s3 sync ../.next/static/ s3://${var.lambda_function_name}-${local.account_id}-${local.region}-static/_next/static/"
    }
